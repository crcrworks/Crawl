...
import { Avatar, Center, Image, Text, View } from 'native-base'
import React, { ReactNode, useMemo } from 'react'
import { Animated, Easing, LayoutAnimation, TouchableOpacity } from 'react-native'
import { GestureEvent, State, TapGestureHandler, TapGestureHandlerEventPayload } from 'react-native-gesture-handler'
    ...
    >
      ...
      <View flexDirection="column" alignItems="flex-start">
        <View bg="black.200" borderTopLeftRadius={5} borderTopRightRadius={20} borderBottomRightRadius={20} borderBottomLeftRadius={20}>
          {appearNote.text && <MkMessage text={parseEmojiCodeToEmoji(<Text>{parseToReactNode(appearNote.text)}</Text>)} />}
        </View>
      </View>
    </MkHeader>
  )
      ...
      >
        <MotiView state={animationState} transition={{ type: 'timing', duration: 200, easing: MotiEasing.ease }}>
          <View flexDirection="row" w="100%" maxW="100%" px={2} py={1}>
            <TouchableOpacity style={{ top: 15, margin: 2, marginRight: 2, height: 40 }} onPress={handlePressProfile}>
              <Avatar source={{ uri: user.avatarUrl }} width="40px" height="40px" borderRadius={100} />
            </TouchableOpacity>
